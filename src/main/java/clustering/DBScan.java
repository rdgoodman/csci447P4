...
public class DBScan {
	...
	public ArrayList<Cluster> run(ArrayList<Datum> training){
		...
		//A Core point is one where there are at least MinPts points within epsilon neighborhood 
		ArrayList<Datum> core = new ArrayList<Datum>();
		
		//A Border point is one that falls within epsilon distance of some Core point 
		...
		//Holds all visited points in the dataset, for constructing the core set
		ArrayList<Datum> coreVisited = new ArrayList<Datum>();
		
		// first train data
		System.out.println(">>>>> TRAINING");
		
		//Build the set of all core points, then perform  DB scan using the core set
		...
		for (Datum d : training) {
			//if the distance between currPt and d is within epsilon, add to border set
		...
		if (core.size() >= minPoints) {
			core.add(currPt);
			//reset currPt to one of the points in the border set
			if (border.size() > 0) {
				currPt = border.get(0);
			}
		} else {
			coreVisited.remove(currPt);
			border.clear();
